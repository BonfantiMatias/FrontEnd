...
export class FormPComponent implements OnInit {
  ...
  create(): void {
    ...
    this.proyectosService.create(this.web).subscribe(
      web =>{
        this.router.navigate(['form4']);
        Swal.fire('Nuevo Proyecto', `El proyecto ${web.nombre} ha sido agregado con exito`,'success')
      }
    );
  }
  ...
  update():void{
    this.proyectosService.update(this.web).subscribe(web => {
      Swal.fire('Informacion Actualizada', `${web.nombre} actualizada con exito!`, 'success')
    })
  }
